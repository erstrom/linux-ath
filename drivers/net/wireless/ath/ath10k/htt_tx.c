...
int ath10k_htt_send_rx_ring_cfg_hl(struct ath10k_htt *htt)
{
	...
	struct sk_buff *skb;
	struct htt_cmd *cmd;
	struct htt_rx_ring_setup_ring_hl *ring;
	const int num_rx_ring = 1;
	u16 flags;
	int len;
	int ret;
	...
	ring->ring_size = __cpu_to_le32(HTT_RX_RING_SIZE_MIN);
	ring->buf_size = __cpu_to_le32(HTT_RX_BUF_SIZE);
	ring->flags = __cpu_to_le16(flags);
