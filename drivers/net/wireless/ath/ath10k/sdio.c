...
static int ath10k_sdio_bmi_credits(struct ath10k *ar)
{
	...
	cmd_credits = kzalloc(4, GFP_KERNEL);
	if (!cmd_credits) {
		ret = -ENOMEM;
		goto err;
	}
...
static int ath10k_sdio_hif_set_mbox_sleep(struct ath10k *ar, bool enable_sleep)
{
	...
	if (enable_sleep)
		*val &= cpu_to_le32(FIFO_TIMEOUT_AND_CHIP_CONTROL_DISABLE_SLEEP_OFF);
	else
		*val |= cpu_to_le32(FIFO_TIMEOUT_AND_CHIP_CONTROL_DISABLE_SLEEP_ON);
