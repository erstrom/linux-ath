...
static void ath10k_usb_start_recv_pipes(struct ath10k *ar)
{
	 ...
	 * ar_usb->pipes[ATH10K_USB_PIPE_RX_CTRL].urb_cnt_thresh =
	 *      ar_usb->pipes[ATH10K_USB_PIPE_RX_CTRL].urb_alloc/2;
	 * ath10k_usb_post_recv_transfers(&ar_usb->
	 *		pipes[ATH10K_USB_PIPE_RX_CTRL],
	 *		ATH10K_USB_RX_BUFFER_SIZE);
	 */
...
static void ath10k_usb_io_comp_work(struct work_struct *work)
{
	...
	while ((skb = skb_dequeue(&pipe->io_comp_queue))) {
		if (pipe->flags & ATH10K_USB_PIPE_FLAG_TX) {
			ath10k_dbg(ar, ATH10K_DBG_USB_BULK,
				   "ath10k usb xmit callback buf:0x%p\n", skb);
			ath10k_usb_tx_complete(ar, skb);
		} else {
			ath10k_dbg(ar, ATH10K_DBG_USB_BULK,
				   "ath10k usb recv callback buf:0x%p\n", skb);
				...
				struct ath10k_hif_sg_item *items, int n_items)
{
	...
	for (i = 0; i < n_items; i++) {
		...
		/* FIXME: Unsure if this is the right place to free the urb.
		 * Perhaps it should be free'd in the transmit complete cb?
		 */
		usb_free_urb(urb);
	}
			    ...
			    const struct usb_device_id *id)
{
	...
	ar->id.device = 0xabcd; /*FIXME: Use sane value*/
...
static int ath10k_usb_pm_resume(struct usb_interface *interface)
{
	...
	ath10k_usb_post_recv_transfers(ar,
				       &ar_usb->pipes[ATH10K_USB_PIPE_RX_DATA]);
	ath10k_usb_post_recv_transfers(ar,
				       &ar_usb->pipes[ATH10K_USB_PIPE_RX_DATA2]);
